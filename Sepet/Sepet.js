...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function sepetPanelAc() {
        ...
        sepetGuncelle(); // Her zaman güncelleyelim
    ...
    function urunAdetGuncelle(urunId, yeniAdet) {
        ...
        if (urun) {
            if (yeniAdet <= 0) {
                ...
                sepetGuncelle();
                sepetSayisiniGuncelle();
                bildirimGoster(`"${urun.ad}" ürününün adeti güncellendi: ${yeniAdet}`);
            }
        }
    }
        ...
        if (sepet.length === 0) {
            ...
            if (sepetUrunler) {
                ...
                sepet.forEach(urun => {
                    ...
                    // İşlem tipi seçimi için select elementi ekle
                    const islemTipiSecimi = `
                        <select class="islem-tipi-select" data-id="${urun.id}">
                            <option value="Satın Alma" ${urun.tip === 'Satın Alma' ? 'selected' : ''}>Satın Alma</option>
                            <option value="Kiralama" ${urun.tip === 'Kiralama' ? 'selected' : ''}>Kiralama</option>
                        </select>
                    `;
                ...
                // İşlem tipi seçim dinleyicileri
                const islemTipleriSelect = sepetUrunler.querySelectorAll('.islem-tipi-select');
                islemTipleriSelect.forEach(select => {
                    select.addEventListener('change', function() {
                        const urunId = parseInt(this.getAttribute('data-id'));
                        const yeniTip = this.value;
...
// İşlem tipini değiştirmek için stil eklemeleri
document.addEventListener('DOMContentLoaded', function() {
    // CSS ekle
